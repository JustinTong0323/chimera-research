...
class RequestHandlerWorkerPool:
    ...
    def __init__(
        ...
        """
        Request handler will not hold push socket to scheduler (model instance or worker),
        It will only send requests to gpu scheduler, and receive responses from gpu scheduler and detokenizers.
        """
        self.server_args = server_args

        enable_gpu_scheduler = server_args.enable_gpu_scheduler
        if not enable_gpu_scheduler:
            raise RuntimeError("GPU scheduler must be enabled when using worker pool.")

        if not server_args.enable_worker_pool:
            raise RuntimeError(
                "Worker pool must be enabled when using worker pool request handler."
            )

        # Init inter-process communication
        num_detokenizer = 1
        num_controller = 1 if controller_ipc_name is not None else 0
        num_gpu_schedulers = num_gpus
        context = zmq.asyncio.Context(
            io_threads=num_detokenizer
            + num_controller
            + num_gpu_schedulers
            + num_engines
        )
        ...
        # for sending generation requests
    ...
    def cleanup(self):
        ...
        try:
            ...
            cleanup_zmq_ipc(
                ...
                component_name="RequestHandlerWorkerPool",
            )
    ...
    def _send_req_to_gpu_scheduler(
        ...
        assert req.gpu_id is not None
        send_to_gpu_scheduler = self.send_to_gpu_scheduler_dict[req.gpu_id]
