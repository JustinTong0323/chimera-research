...
class RequestHandler:
    ...
    def __init__(
        ...
        raise RuntimeError("Should not call tokenizer_mangager.RequestHandler")
        self.server_args = server_args

        # Init inter-process communication
        context = zmq.asyncio.Context(1)
        self.recv_from_detokenizer = context.socket(zmq.PULL)
        self.recv_from_detokenizer.bind(f"ipc://{port_args.request_handler_ipc_name}")
        ...
        # generation responses will be sent by detokenizer into redis with the key of `generate_response_key`, responses of all models will be put into the same queue.
        self.generate_response_key = server_args.generate_response_key
    ...
    async def _send_single_request(
        ...
        await self.redis_client.send_pyobj(
            key=f"{self.generate_request_key_prefix}:{model}", obj=single_request_obj
        )
        print(
            f"TokenizerManager: Sent request to {model} with rid {single_request_obj.rid} into redis with key of {self.generate_request_key_prefix}:{model}"
        )
    ...
    async def handle_loop(self):
        ...
        while True:
            ...
            ] = await self.redis_client.recv_pyobj_block(self.generate_response_key)
            print(
                f"TokenizerManager: Received response from redis with key of {self.generate_response_key}, rids are {recv_obj.rids}"
            )
