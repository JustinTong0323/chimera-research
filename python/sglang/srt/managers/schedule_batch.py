...
class Req:
    ...
    def __init__(
        ...
        self.slo = slo
        self.out_queue_timestamp = None
        self.prefill_finish_timestamp = None
        self.decode_timestamps = (
            []
        )  # every decode stage's timestamp, include the finish timestamp for the last decode token
...
class ScheduleBatch:
    ...
    def alloc_token_slots(self, num_tokens: int):
        ...
        # retry 20 times if cannot allocate memory
    ...
    def check_decode_mem(self):
        ...
        if self.token_to_kv_pool.available_size() >= bs:
            # success = self.token_to_kv_pool.try_to_reserve(bs)
            # if success:
            #     return True
            return True
    ...
    def retract_decode(self):
        ...
        # TODO: improve retraction policy for radix cache
        sorted_indices.sort(
            key=lambda i: (
        ...
        first_iter = True
        retract_decode_steps = global_config.retract_decode_steps
        ...
        while (
            ...
            if len(sorted_indices) == 1 and retract_decode_steps > 1:
                logger.warning(
                    "RetractDecode: Only one request in the batch, sleep a while and try again with half steps"
                )
            ...
            if isinstance(self.tree_cache, ChunkCache):
                ...
                residual_size = (
                    len(sorted_indices) * retract_decode_steps
                    - self.token_to_kv_pool.available_size()
                )
        ...
        if total_max_new_tokens == 0:
            ...
            logging.warning("Total max_new_tokens is 0, set new_estimate_ratio to 0.0")
        else:
            new_estimate_ratio = (
                total_decoded_tokens + retract_decode_steps * len(self.reqs)
