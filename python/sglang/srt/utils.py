...
def memory_stats():
    print(f"Allocated: {torch.cuda.memory_allocated()/1024**2:.2f} GB")
    print(f"Reserved: {torch.cuda.memory_reserved()/1024**2:.2f} GB")
...
def configure_logger(server_args, prefix: str = "", log_file_suffix: str = ""):
    format = f"[%(asctime)s.%(msecs)03d{prefix}] %(message)s"
    if server_args.log_file:
        filename = (
            server_args.log_file + f".{log_file_suffix}.log"
            if log_file_suffix
            else server_args.log_file
        )
    else:
        filename = None
...
def change_logger_format(prefix: str = ""):
    format = f"[%(asctime)s.%(msecs)03d{prefix}] %(message)s"
...
def get_model_name_to_cell_size(model_names_to_paths: Dict[str, str]) -> Dict[str, int]:
    """Get the mapping between model names and their cell sizes.

    Args:
        model_names_to_paths: A dictionary mapping model names to their model paths.

    Returns:
        A dictionary mapping model names to their cell sizes in bytes.
    """
    cur_dir = os.path.dirname(os.path.abspath(__file__))
    model_info_path = os.path.join(cur_dir, "../multi_model/utils/model_info.json")
    with open(model_info_path, "r") as f:
        model_info = json.load(f)

    model_name_to_cell_size = {}
    for model_name, model_path in model_names_to_paths.items():
        if model_path not in model_info:
            raise ValueError(
                f"Model path {model_path} not found in the profiled model info file."
            )
        model_name_to_cell_size[model_name] = model_info[model_path]["cell_size"]

    return model_name_to_cell_size


def wait_for_cuda(module, check_interval=0.01):
    """Wait until the module's parameters are moved to CUDA device.

    Args:
        module: The PyTorch module to check
        check_interval: Time interval between checks in seconds
    """
    while "cuda" not in str(next(module.parameters()).device):
