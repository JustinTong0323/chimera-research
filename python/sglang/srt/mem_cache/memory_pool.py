    ...
    def __init__(
        ...
        if enable_elastic_memory:
            try:
                ...
                from emm.slab_allocator import KVCacheManager

                self.emm_ops = emm_ops
                self.kv_cache_manager = KVCacheManager

                if not enable_worker_pool:
                    self.emm_ops.init_emm(self.gpu_id)

                self.init_kv_allocator()
            except ImportError as e:
                raise ImportError(
                    "emm package is required for elastic memory. Please install it first."
                ) from e
        else:
            self._init_kv_cache(self.min_reserve_mem)

    def init_kv_allocator(self):
        k_buffer, v_buffer = self.emm_ops.sgl_alloc_kv_cache(
            self.size,
            self.head_num,
            self.head_dim,
            self.dtype,
        ...
        if not self.enable_elastic_memory:
            ...
            if self.use_emm_v0:
                ...
                if self.enable_worker_pool:
                    ...
                    self.emm_ops.free_kv_cached_tensors()
            else:
                self.kv_allocator.clear()

    def shutdown(self):
        if self.enable_elastic_memory:
            self.emm_ops.shutdown_emm()
            del self.kv_allocator
            self.k_buffer = None
            self.v_buffer = None
