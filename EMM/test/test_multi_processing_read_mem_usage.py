...
from emm.ops import init_emm, shutdown_emm
...
def run_kv_cached(i):
    ...
    print(f"Initializing emm in process {i}")
    init_emm()
    print(f"Initialized emm in process {i}")
    k_buffer, v_buffer = emm_ops.sgl_alloc_kv_cache(
        total_tokens,
        head_num,
        head_dim,
        dtype,
