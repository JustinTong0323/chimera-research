...
from emm.slab_allocator import PAGE_SIZE

try:
    from emm.vmm_ops import create_kv_tensors, free_kv_tensors
    from emm.vmm_ops import init_emm as _init_emm_impl
    from emm.vmm_ops import shutdown_emm as _shutdown_emm_impl
except ImportError:
    # If direct import fails, try importing from vmm_ops module
    # This handles the case where emm is used as a standalone module
    try:
        from vmm_ops import create_kv_tensors, free_kv_tensors
        from vmm_ops import init_emm as _init_emm_impl
        from vmm_ops import shutdown_emm as _shutdown_emm_impl
    except ImportError:
        ...
        EMM_PATH = os.path.realpath(f"{SCRIPT_PATH}/../csrc")
        if os.path.exists(EMM_PATH):
            sys.path.append(EMM_PATH)

        # Try to import from emm.vmm_ops or vmm_ops
        try:
            from emm.vmm_ops import create_kv_tensors, free_kv_tensors
            from emm.vmm_ops import init_emm as _init_emm_impl
            from emm.vmm_ops import \
                shutdown_emm as _shutdown_emm_impl
        except ImportError:
            from vmm_ops import create_kv_tensors, free_kv_tensors
            from vmm_ops import init_emm as _init_emm_impl
            from vmm_ops import shutdown_emm as _shutdown_emm_impl
...
def init_emm(gpu_id: Optional[int] = None,
    ...
    _init_emm_impl(virtual_mem_size, device, reserve_virtual_mem)


def shutdown_emm() -> None:
    _shutdown_emm_impl()
